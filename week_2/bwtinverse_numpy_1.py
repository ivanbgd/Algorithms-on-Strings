...
def create_matrix(bwt):
    ...
    for row, c in enumerate(sorted_bwt):
        ...
        occurrences[(bytes(current_letter, encoding="ascii"), cpos)] = row
...
def inverse_bwt(bwt):
    ...
    for i in range(dim):
        ...
        # result[dim-1-i] = str(current_letter.char, encoding="ascii")
        result[dim-1-i] = current_letter.char.decode("ascii")
        next_letter = m[row][1]
