...
def create_matrix(bwt):
    dim = len(bwt)
    m = np.recarray(shape=(dim, 2), dtype=[("char", "U1"), ("pos", "int")])
    occurrences = {}
...
def inverse_bwt(bwt):
    ...
    current_letter = m[0][0]  # ('$', 0); dtype is: (numpy.record, [('char', 'U1'), ('pos', '<i4')]); type is: <class 'numpy.record'>
    row = occurrences[(current_letter.char, current_letter.pos)]
    for i in range(dim):
        # print(f"\ni = {i}: current_letter = {current_letter}, row = {row}")
        result[dim-1-i] = current_letter.char
        next_letter = m[row][1]
        row = occurrences[(next_letter.char, next_letter.pos)]
